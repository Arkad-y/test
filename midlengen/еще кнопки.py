...
def create_keyboard(response):
    ...
    if response == 'клава':
        ...
        keyboard.add_button('нажимай, чтобы управлять', color=VkKeyboardColor.NEGATIVE)
...
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW:
        ...
        if event.from_user and not event.from_me:
            if response == "нажимай, чтобы управлять":
                send_message(vk_session, 'user_id', event.user_id, message='Нажми на кнопку', keyboard=keyboard)
            elif response == "клава":
                send_message(vk_session, 'user_id', event.user_id, message= 'Жми',keyboard=keyboard)

            elif response=='закрыть':
                send_message(vk_session, 'user_id', event.user_id, message='Закрываю',keyboard=keyboard)